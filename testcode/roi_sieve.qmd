...
### Compute and threshold autocorrelations
...
Now we can use the correlation coefficient of each ROIs autocorrelation and threshold it to get the cells that reliably reacted to something in the stimulus.

Instead of simply using a hard threshold, we can estimate the probability density function of the correlation coefficients and use the ones that are significantly different from the mean.

We first plot a histogram and 