...
from modules.dataloader_oldie import all_rois
...
good_recs2_tec = [0,1,2,4,5]
good_recs2_pretec = [6,9]
good_recs_all = np.arange(len(f.recordings()))

good_recs = good_recs2_pretec
...
# if os.path.exists(dataroot+dataname) == False:
#     d.stimulus_means()
#     np.save(dataroot+dataname, d.mean_dffs)
#     np.save(dataroot+timename, d.mean_times)
# else:
#     d.mean_dffs = np.load(dataroot+dataname)
#     d.mean_times = np.load(dataroot+timename)

d.stimulus_means()
```

To get an overview, we plot all mean dffs in a heatmap.
...
extent = (np.min(d.times), np.max(d.times), 0, len(d.mean_dffs[:,0]))
ax.imshow(d.mean_dffs, aspect='auto', extent=extent, origin='upper')
