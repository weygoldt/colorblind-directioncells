...
def plot_lineplot(ax, mf):
    ...
    for i, roi in enumerate(range(n+1)):
        dff = temp_zscores[i, :]
        ax.plot(mf.times, i + (dff - np.min(min) ) / (np.max(max) - np.min(min)), color='black', linewidth='1.')
    ax.set_xlabel("Time [s]", fontsize= 15)
    ax.set_ylabel("ROIs",     fontsize= 15)
    ax.set_yticks(np.arange(0, n+0.1, 2))
    ax.set_xticks(np.arange(0,1751 , 250))

    ax.set_yticklabels(np.arange(0, n+0.1, 2), fontsize=13)

    ax.set_xticklabels(np.round(np.arange(0,1751 , 250)/60, 1), fontsize=13)
