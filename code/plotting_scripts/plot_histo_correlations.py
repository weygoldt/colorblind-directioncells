...
def plot_lineplot(ax, mf):
    ...
    for i, roi in enumerate(range(100, 100 + n+1, 1)):
        dff = temp_zscores[roi, :]
        ax.plot(mf.times/60, i + (dff - np.min(min)) /
    ...
    ax.set_xticklabels(
        np.round(np.arange((mf.times[0])/60, 30.1, 5), 1), fontsize=13)
...
# make a histpgram
counts, edges = np.histogram(corrs, bins=50, range=(-0.2, 1), density=True)

# compute a gaussian KDE
xkde, kde = fs.kde1d(corrs, 0.02, xlims=[-0.2, 1])
...
fig, ax = plt.subplots(figsize=(12*ps.cm, 10*ps.cm), constrained_layout=True)
...
ax.plot([thresh, thresh], [0, 3.5], lw=1, ls="dashed", c="k")
...
fig, ax = plt.subplots(1, 2, figsize=(9*ps.cm, 6*ps.cm),
